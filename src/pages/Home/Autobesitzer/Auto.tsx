import React, { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';

import I_Benutzer from '../../../services/Types/Benutzer/I_Benutzer';
import { RootState } from '../../../services/Reduces/redux';
import Modal_Auto from '../../../components/Modal/Modal_Auto';
import { deleteAuto__Hilfe } from '../../../services/Reduces/Benutzer/auto_Slice';
import I_Auto from '../../../services/Types/Benutzer/I_Auto';
import Profil from '../../../components/Profil/Profil';
import I_CurrentUser from '../../../services/Types/Benutzer/I_CurrentUser';
import { update_Current_User } from '../../../services/utilities';
import { useNavigate } from 'react-router-dom';

const Auto = () => {
  const dispatch = useDispatch(); //je parle a redux
  const navigate = useNavigate();

  const benutzer_Liste = useSelector((state: RootState) => state.benutzer);
  const lagers = useSelector((state: RootState) => state.lager);
  const lagerplatzreservierungen = useSelector(
    (state: RootState) => state.lagerplatzreservierung
  );
  const services = useSelector((state: RootState) => state.service);
  const beantragte_Services = useSelector(
    (state: RootState) => state.beantragte_Service
  );
  const autos = useSelector((state: RootState) => state.autos);

  const termine = useSelector((state: RootState) => state.termin);

  const [fertigAuto, setFertigAuto] = useState<boolean>(true);

  const [ich, setIch] = useState<I_Benutzer>();
  const [current_User, setCurrent_User] = useState<I_CurrentUser>();

  useEffect(() => {
    const storedUserJSON = localStorage.getItem('currentUser');
    if (storedUserJSON) {
      const storedUser = JSON.parse(storedUserJSON);

      setCurrent_User(storedUser);

      const user = benutzer_Liste.find((el) => el.id === storedUser.aktuell_U);

      setIch(user as I_Benutzer);
    } else {
      console.log("Aucun utilisateur n'est stocké dans le localStorage.");
      navigate('/');
    }
  }, []);

  const bearbeitenA = (id_A: string) => {
    if (ich) {
      update_Current_User(
        ich.id,
        '',
        '',
        id_A,
        '',
        lagers,
        lagerplatzreservierungen,
        services,
        beantragte_Services,
        autos,
        termine
      );

      setFertigAuto(false);
    }
  };

  const loeschenA = async (id: string) => {
    dispatch(await deleteAuto__Hilfe(id));
  };
  /* ----------------------------------------------------------------- */

  return (
    <>
      <Profil />

      <div className="flex pt-16 flex-wrap mb-2">
        <div className="w-full">
          {fertigAuto && (
            <div className="text-center pb-10 pt-5">
              <button
                onClick={() => setFertigAuto(false)}
                type="button"
                className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                Ein Auto erstellen
              </button>
            </div>
          )}

          {!fertigAuto && <Modal_Auto setFertigAuto={setFertigAuto} />}

          <h1 className="text-center text-blue-800 font-bold text-3xl">
            {autos.length === 0 ? 'Noch kein Auto...' : 'Hier sind Ihre Autos'}
          </h1>

          <div className="flex justify-center mt-10">
            <ul className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {autos.map(
                (auto: I_Auto) =>
                  ich &&
                  auto.id_Benutzer === ich.id && (
                    <li className="list-none" key={auto.id}>
                      <div className="max-w-full w-full lg:max-w-full lg:flex  rounded-3xl pb-3">
                        <div
                          className="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-s-3xl lg:rounded-l text-center overflow-hidden"
                          style={{
                            backgroundImage: `url('/imgs/services/service1.jpg')`,
                          }}
                          title="profil"></div>
                        <div className="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                          <div className="mb-8">
                          <p className="text-gray-700 text-base">
                              {auto.name} (Identifier)
                            </p><p className="text-gray-700 text-base">
                              Marke: {auto.marke}.
                            </p>
                            <p className="text-gray-700 text-base">
                              Farbe: {auto.farbe}.
                            </p>
                          </div>
                          <div className="flex items-center">
                            <div className="text-sm">
                              <p className="text-gray-900 leading-none">
                                Von <strong>{ich && ich.name}</strong> gepostet.
                              </p>
                            </div>
                          </div>
                          <div className="flex mt-3">
                            {/* Bouton de Suppression */}

                            <button
                              onClick={() => loeschenA(auto.id)}
                              className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-1 rounded-full mr-2">
                              <svg
                                className="w-4 h-4"
                                fill="none"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                strokeWidth="2"
                                viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path d="M6 18L18 6M6 6l12 12"></path>
                              </svg>
                            </button>
                            {/* Bouton de Modification */}
                            <button
                              onClick={() => bearbeitenA(auto.id)}
                              className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-1 rounded-full">
                              <svg
                                className="w-4 h-4"
                                fill="none"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                strokeWidth="2"
                                viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path d="M15 12a3 3 0 11-6 0 3 3 0 016 0zM19 19a2 2 0 01-2 2H7a2 2 0 01-2-2V7a2 2 0 012-2h5m2-2h3a2 2 0 012 2v3"></path>
                              </svg>
                            </button>
                          </div>
                        </div>
                      </div>
                    </li>
                  )
              )}
            </ul>
          </div>
        </div>
      </div>
    </>
  );
};

export default Auto;
